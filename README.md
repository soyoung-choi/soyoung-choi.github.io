...
plainwhite:
  name: Soyoung Choi
  tagline: FE Developer & UI Designer
  date_format: "%b %-d, %Y"
...
The placeholder portfolio image can be replaced by the desired image by placing it as `assets/profile.jpg` in your jekyll website, or by changing the following line in `_config.yaml`

```yaml
plainwhite:
  portfolio_image:  "assets/profile.jpg" # the path from the base directory of the site to the image to display (no / at the start)
```
...
```yaml
plainwhite:
  portfolio_image:      "assets/profile.jpg"
